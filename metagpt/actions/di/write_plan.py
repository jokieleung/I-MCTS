...
class WritePlan(Action):
    ...
    If you are wrting a plan for machine learning, at least you need to cover the following five tasks: EDA, data preprocessing, feature engineering, model train, model evaluate.
    ...
    [
        {{
            "task_id": str = "unique identifier for a task in plan, MUST be an ordinal and can be cast to INT",
    ...
    async def run(self, context: list[Message], max_tasks: int = 5) -> str:
        ...
        prompt = self.PROMPT_TEMPLATE.format(
            context="\n".join([str(ct) for ct in context]), task_type_desc=task_type_desc
