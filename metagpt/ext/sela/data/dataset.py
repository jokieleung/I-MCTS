...
5. When feature enginnering, make sure NEVER use the target column.
6. When scaling or transforming features, make sure the target column is NOT included.
7. You could utilize dev set to validate and improve model training. {special_instruction}

## Saving Dev and Test Predictions, and the MODEL you train
...
3. Save the trained model files in `model.pkl` in the output directory. 

## Output Performance
Print the train, dev, and test set performance in the last step. Please don't generate meaningless code, such as print of some reflective thoughts.
...
CUSTOM_DATASETS = [
    # ("ccFraud", "fraudRisk"),
    ("lending_club", "Default"),
    # ("polish_companies_bankruptcy_data", "class"),
    # ("Travel_Insurance", "Claim"),
    # ("Taiwan_Economic_Journal", "Bankrupt?"),
    # ("Credit_Card_Fraud_Detection", "Class"),
    # ("statlog+australian+credit+approval", "target"),
    # ("statlog+german+credit+data", "target"),
    # ("PortoSeguro", "target"),
    # ("jingying_score_prediction_small_0107", "label"),
    # ("Click_prediction_small", "click"),
...
if __name__ == "__main__":
    ...
    for dataset_name, target_col in CUSTOM_DATASETS:
        try:
            print(os.path.join(datasets_dir, dataset_name,  'dataset_description.txt'))
            with open(os.path.join(datasets_dir, dataset_name,  'dataset_description.txt'), 'r') as file:
                dataset_description = file.read()
                print(dataset_description)
        except:
            dataset_description = ""
        custom_dataset = ExpDataset(dataset_name, datasets_dir, target_col=target_col, force_update=force_update, dataset_description=dataset_description)
        asyncio.run(process_dataset(custom_dataset, solution_designer, save_analysis_pool, datasets_dict))
        save_datasets_dict_to_yaml(datasets_dict, str(dataset_name)+ ".yaml")
