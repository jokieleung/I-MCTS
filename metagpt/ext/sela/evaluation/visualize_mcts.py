...
def build_tree_recursive(graph, parent_id, node, node_order, start_task_id=2):
    ...
    if depth == 0:
        ...
        id = (depth + start_task_id - 1) if (depth + start_task_id - 1) < len(role.planner.plan.tasks) else len(role.planner.plan.tasks) - 1
        instruction = role.planner.plan.tasks[id].instruction
    print(instruction)
    # Add the current node with attributes to the graph
    dev_score = node.raw_reward.get("dev_score", 0) * 100
    test_score = node.raw_reward.get("test_score", 0) * 100
    avg_score = node.avg_value() * 100
    order = node_order.index(node.id) if node.id in node_order else ""
    print(f"{node.id}\nAvg: {avg_score:.1f}\nDev_Score: {dev_score:.1f}\nTest_Score: {test_score:.1f}\nVisits: {node.visited}\nOrder: {order}")
    # graph.add_node(
    #     parent_id,
    #     label=f"{node.id}\nAvg: {avg_score:.1f}\nDev_Score: {dev_score:.1f}\nTest_Score: {test_score:.1f}\nVisits: {node.visited}\nOrder: {order}",
    #     avg_value=node.avg_value(),
